  ...
  const outcome = await withTx(deps.pool, async (c) => {
    ...
    if (d.endpoint.status === "disabled") {
      ...
      try {
        ...
        // the attempt, with no HTTP call, if any answer is not public. Skipped only in
        // tests, whose receiver deliberately lives on 127.0.0.1.
        if (deps.config.NODE_ENV !== "test") {
          ...
          if (resolved.some((r) => !isPublicAddress(r.address))) {
            throw new PrivateAddressError("destination resolves to a private address");
          }
        }
        ...
        excerpt = err instanceof PrivateAddressError ? err.message : `request failed: ${(err as Error).name}`;
      } finally {
        clearTimeout(timer);
      }
    }
